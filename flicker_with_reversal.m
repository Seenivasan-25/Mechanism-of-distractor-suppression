...
try
    ...
    conds = readtable("Inputfile_experiment2.xlsx");    
    conds = conds(randperm(height(conds)),:);
    N = height(conds);
    
    % Check if we have exactly 540 trials
    if N ~= 540
        error('Input file should contain exactly 540 trials. Current file has %d trials.', N);
    end
    ...
    Practice_ShapeSingletonIndex = []; Practice_ColorSingleton = []; Practice_ShapeSingleton = [];
    Practice_ResponseKey = {}; Practice_feedback = []; Practice_searchRT = [];Practice_ColorSingletonIsTarget=[]; Practice_NumItems = [];
    Practice_SetSize = [];
 
    % Data storage for main experiment
    Subj = []; TrialNo = []; TargetIndex = []; ColorSingletonIndex = []; ShapeSingletonIndex = [];
    ColorSingleton = []; ShapeSingleton = [];
    ResponseKey = {}; feedback = []; searchRT = [];ColorSingletonIsTarget=[]; NumItems = [];
    SetSize = [];
    ...
    if ~skip_practice
        ...
        shape_singleton_vals = [zeros(10,1); ones(10,1)]; % 10 each of 0 and 1
        ...
        practice_shape_singleton = shape_singleton_vals(practice_order);
        ...
        for practice_trial = 1:20
            ...
            shape_singleton_code = practice_shape_singleton(practice_trial);
            ...
            if shape_singleton_code == 1 % Shape singleton present
               ...
               % shape_singleton_index = opts(randi(length(opts)));
                ...
                elseif color_singleton_index==num_items
                    ...
                    adjacent_postions=[num_items-1,1];
                end
                opts=setdiff(opts,[adjacent_positions,color_singleton_index]);
                if ~isempty(opts)
                    shape_singleton_index=opts(randi(length(opts)));
                else
                    all_opts=setdiff(1:num_items,[target_index,color_singleton_index]);
                    shape_singleton_index=all_opts(randi(length(all_opts)));
                end
            else % Shape singleton absent
                shape_singleton_index = -1;
            end
...
if shape_singleton_code == 1
    ...
    Screen('Flip', wPtr); WaitSecs(0.24);
           ...
           % Store practice trial data
            ...
            if shape_singleton_code == 1
                Practice_ShapeSingletonIndex = [Practice_ShapeSingletonIndex; shape_singleton_index];
            else
                Practice_ShapeSingletonIndex = [Practice_ShapeSingletonIndex; NaN];
            end
            Practice_ColorSingleton = [Practice_ColorSingleton; color_singleton_code];
            Practice_ShapeSingleton = [Practice_ShapeSingleton; shape_singleton_code];
            ...
            Practice_ColorSingletonIsTarget=[Practice_ColorSingletonIsTarget;(color_singleton_code==0)];
    ...
    main_experiment_instructions = ['Main Experiment - 540 Trials\n\n'...
                                  'You will now begin the main experiment.\n\n'...
                                  'ERROR TRIALS WILL BE REPEATED UNTIL ALL 540 TRIALS ARE CORRECT.\n\n'...
    ...
    while completed_trials < 540
        % Check for break every 72 completed trials
        if mod(completed_trials, 90) == 0 && completed_trials > 0
            break_text = sprintf('Break time!\n\nYou have completed %d correct trials.\n\nTake a 30-second rest.\n\nThe experiment will continue automatically.', completed_trials);
            DrawFormattedText(wPtr, break_text, 'center', 'center', [255 255 255]);
            Screen('Flip', wPtr);
            
            % 30-second countdown
            for countdown = 30:-1:1
                countdown_text = sprintf('Break time!\n\nYou have completed %d correct trials.\n\nTake a 30-second rest.\n\nResuming in: %d seconds', completed_trials, countdown);
        ...
        shape_singleton_code = conds.ShapeSingleton(current_trial_index);  % 0=absent, 1=present
...
if shape_singleton_code == 1
 ...
 if shape_singleton_code == 1
 ShapeSingletonIndex = [ShapeSingletonIndex; shape_singleton_index];
 else
 ShapeSingletonIndex = [ShapeSingletonIndex; NaN];
 end
 ColorSingleton = [ColorSingleton; color_singleton_code];
 ShapeSingleton = [ShapeSingleton; shape_singleton_code];
 ResponseKey = [ResponseKey; {resp}];
 feedback = [feedback; isCorrect];
 searchRT = [searchRT; RT];
 ColorSingletonIsTarget=[ColorSingletonIsTarget;(color_singleton_code==0)];
 ...
 ShapeSingletonIndex, ColorSingleton, ShapeSingleton, ...
 ...
 final_accuracy, mean_RT, trial-1, completed_trials);
